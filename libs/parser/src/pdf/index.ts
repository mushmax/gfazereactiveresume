...
export class PdfParser implements Parser<File, PdfData> {
  schema: Schema;
  ...
  validate(_data: File): Promise<PdfData> {
    try {
      return Promise.resolve(
        this.schema.parse({
          text: "PDF parsing not yet implemented - please use JSON import for now",
          metadata: {},
        }),
      );
    } catch (error) {
